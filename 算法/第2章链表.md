...
### 题解
1. 递归法
   不断地递归，如果碰到list1 < list2的情况，那么就说明list1的这个节点应该在前面，而它的next节点应当是list2和list1->next的较小值，所以直接list1->next = mergeTwoLists(list1->next,list2);就可以递归下去，结束条件就是list1或list2为null的时候，此时直接返回另一个节点就好。
2. 迭代法
   迭代法更加地好理解，就是使用一个虚拟头节点，然后定义一个不断向后的节点，判断list1和list2的val大小，将小的一个插入，不断遍历知道其中一个链表为空，然后将另一个链表剩下的全部插入。
### 代码
递归法
```cpp
class Solution {
